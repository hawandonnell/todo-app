...
function deleteGroup() {
    let groupToDelete = todosStore.groups.find(
        (el) => el.id == todosStore.activeGroupIndex
    )
    todosStore.groups.splice(todosStore.groups.indexOf(groupToDelete), 1)
    todosStore.tasks = todosStore.tasks.filter(
        (task) => task.userId != todosStore.activeGroupIndex
    )
    if (todosStore.groups.length) {
        todosStore.activeGroupIndex = todosStore.groups[0].id
    } else {
        todosStore.activeGroupIndex = -1
